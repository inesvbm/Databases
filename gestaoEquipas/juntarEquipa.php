				    ...
				    <?php
						...
						try{
							...
							if ($connection->connect_errno != 0){
					    		...
					    		echo "<th><strong>Cartão de cidadão</strong></th>
					    		...
					    		if(isset($_POST['submit'])){
					    			...
					    			if($cc==null){
					    				...
					    				$torneio=$connection->query("SELECT COUNT(*)
																FROM torneios
																WHERE torneios.Nome_torneio NOT IN (SELECT DISTINCT jogadores_jogo.Nome_torneio
																									FROM jogadores_jogo
																									WHERE jogadores_jogo.CC LIKE '".$cc."')");

					    				$conta=mysqli_fetch_row($torneio);

					    				if($conta[0]==0){
					    					echo '<script language="javascript">';
											echo 'alert("Já inscrito em todos os torneios!")';
											echo '</script>';
					    				}
					    				else
					    					header('Location: juntarEquipa1.php?cc='.$cc.'');
					    			}
					    		}
