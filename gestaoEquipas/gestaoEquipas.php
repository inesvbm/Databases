						...
						try{
							...
							if ($connection->connect_errno != 0){
					    		...
					    		echo "<form action='' method='post'>
					    			<th><strong>Cartão de cidadão</strong></th>
					    			<input type='text' name='cc' style='height:30px;'/><br>";

					    		echo "<input type='submit' name='button' value='Minhas equipas' style='width:199px;'>";
					    		...
					    		if(isset($_POST['button'])){
					    			...
					    			$jaInscrito=$connection->query("SELECT COUNT(*)
																	FROM utilizadores
																	WHERE utilizadores.CC LIKE '".$cc."'");

					    			$utilizadorExiste=mysqli_fetch_row($jaInscrito);

					    			if($cc==null){
					    				echo '<script language="javascript">';
										echo 'alert("Deve inserir o número de identificação!")';
										echo '</script>';
					    			}
					    			else if($utilizadorExiste[0]==0){
					    				echo '<script language="javascript">';
					    				...
					    				///////////////////////////////////// VERIFICA SE UTILIZADOR É CAPITÃO /////////////////////////////////////////////////

					    				$temEquipas=$connection->query("SELECT COUNT(*)
																	FROM equipas
																	WHERE equipas.CC LIKE '".$cc."'");
					    				$conta=mysqli_fetch_row($temEquipas);

					    				if($conta[0]==0){
											echo '<script language="javascript">';
											echo 'alert("Não é capitão de nenhuma equipa!")';
											echo '</script>';
					    				}
					    				else{
					    					$nomeEquipa=$connection->query("SELECT equipas.Nome_equipa, equipas.Nome_torneio, torneios.Data_inicio, torneios.Data_fim   
					    										FROM equipas, torneios
					    										WHERE equipas.CC LIKE '".$cc."'
					    										AND equipas.Nome_torneio LIKE torneios.Nome_torneio
					    										ORDER BY equipas.Nome_equipa");

					    					///////////////////////////////////// TABELA DE EQUIPAS DO CAPITÃO /////////////////////////////////////////////////
									        ...
									        while($equipas=mysqli_fetch_array($nomeEquipa)){
												$nome=$equipas[0];
												echo '<tr>
											             <td>'.$nome.'</td>
											             <td>'.$equipas[1].'</td>
											             <td>'.$equipas[2].'</td>
											             <td>'.$equipas[3].'</td>
											             <?php $_SESSION["equipa"]=$nome ?>
											             <td><input type="button" value=">" onclick="location.href=\'gestaoEquipas1.php?nomeEquipa='.$nome.'  '.$equipas[1].'\'"></td> 
											           </tr>'; 
											}
										    echo "</table>";
										    echo"</form>";
